...
```bash
node .pi/skills/impeccable-flutter/scripts/live.mjs
```
...
```
LOOP:
  node .pi/skills/impeccable-flutter/scripts/live-poll.mjs   # default long timeout; no --timeout=
...
```bash
node .pi/skills/impeccable-flutter/scripts/live-status.mjs
node .pi/skills/impeccable-flutter/scripts/live-resume.mjs --id SESSION_ID
node .pi/skills/impeccable-flutter/scripts/live-complete.mjs --id SESSION_ID
```
...
```bash
node .pi/skills/impeccable-flutter/scripts/live-wrap.mjs --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div" --text "TEXT_SNIPPET"
```
...
```bash
node .pi/skills/impeccable-flutter/scripts/live-poll.mjs --reply EVENT_ID done --file RELATIVE_PATH
```
...
```bash
node .pi/skills/impeccable-flutter/scripts/live-poll.mjs --reply EVENT_ID error "Short reason"
```
...
```bash
node .pi/skills/impeccable-flutter/scripts/live-server.mjs stop
```
...
```bash
node .pi/skills/impeccable-flutter/scripts/detect-csp.mjs
```
