/**
 * Shared context loader for every impeccable-flutter command that needs to know
 ...
 *     migrated: boolean,          // true if we auto-renamed .impeccable-flutter.md -> PRODUCT.md
 ...
 *   2. cwd, if PRODUCT.md / DESIGN.md / .impeccable-flutter.md is there (back-compat)
 *   3. Auto-fallback subdirectories of cwd: .agents/context/, then docs/
 *   4. cwd as a default "no context found" location
 *
 * Legacy `.impeccable-flutter.md` -> PRODUCT.md migration only fires at cwd root;
...
const LEGACY_NAMES = ['.impeccable-flutter.md'];
...
export function resolveContextDir(cwd = process.cwd()) {
  ...
  //    legacy `.impeccable-flutter.md` does not pull the lookup into a fallback dir.
...
export function loadContext(cwd = process.cwd()) {
  ...
  // 2. Legacy: if no PRODUCT.md but .impeccable-flutter.md exists at cwd root, rename
