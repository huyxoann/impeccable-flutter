 ...
 *
 * `pin audit` creates a lightweight /audit skill that redirects to /impeccable-flutter audit.
...
/**
 * Find harness skill directories that have an impeccable-flutter skill installed.
 */
function findHarnessDirs(projectRoot) {
  const dirs = [];
  for (const harness of HARNESS_DIRS) {
    const skillsDir = join(projectRoot, harness, 'skills');
    // Only pin in harness dirs that already have impeccable-flutter installed
    const impeccable-flutterDir = join(skillsDir, 'impeccable-flutter');
    if (existsSync(impeccable-flutterDir) || existsSync(join(skillsDir, 'i-impeccable-flutter'))) {
      dirs.push(skillsDir);
    }
  }
  return dirs;
...
function generatePinnedSkill(command, metadata) {
  const desc = metadata[command]?.description || `Shortcut for /impeccable-flutter ${command}.`;
...
This is a pinned shortcut for \`{{command_prefix}}impeccable-flutter ${command}\`.

Invoke {{command_prefix}}impeccable-flutter ${command}, passing along any arguments provided here, and follow its instructions.
`;
}
...
function pin(command, projectRoot) {
  ...
  if (harnessDirs.length === 0) {
    console.log('No harness directories with impeccable-flutter installed found.');
    return false;
  }
...
function unpin(command, projectRoot) {
  ...
  if (removed > 0) {
    ...
    console.log(`Use /impeccable-flutter ${command} to access it.`);
  } else {
