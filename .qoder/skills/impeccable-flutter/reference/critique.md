...
2. **Compute the slug.** Run:
   ```bash
   node .qoder/skills/impeccable-flutter/scripts/critique-storage.mjs slug "<resolved-path-or-url>"
   ```
...
2. **Pass the structured metadata**:
   ...
   IMPECCABLE_CRITIQUE_META='{"target":"<user phrasing>","total_score":<n>,"p0_count":<n>,"p1_count":<n>}' \
     node .qoder/skills/impeccable-flutter/scripts/critique-storage.mjs write <slug> <body-file>
   ```
3. **Read the trend**:
   ```bash
   node .qoder/skills/impeccable-flutter/scripts/critique-storage.mjs trend <slug> 5
   ```
