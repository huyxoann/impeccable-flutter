 ...
 *      "impeccable-flutter" (to avoid deleting unrelated user skills).
...
const DEPRECATED_NAMES = [
  // v2.0 renames
  'frontend-design',    // renamed to impeccable-flutter
  'teach-impeccable-flutter',   // folded into /impeccable-flutter teach
  // v2.1 merges
  'arrange',            // renamed to layout
  'normalize',          // merged into polish
  'onboard',            // merged into harden
  'extract',            // merged into /impeccable-flutter extract
  // v3.0 consolidation: all standalone skills -> /impeccable-flutter sub-commands
  'adapt',
  'animate',
  'audit',
  'bolder',
...
// "impeccable-flutter" in their v2.x source. Used as a last-resort match
 ...
 *   1. Lock source equals "pbakaus/impeccable-flutter" (authoritative).
 *   2. SKILL.md body contains the word "impeccable-flutter".
...
export function isImpeccableSkill(skillDir, { skillName, lock } = {}) {
  ...
  if (skillName && lock?.skills?.[skillName]?.source === 'pbakaus/impeccable-flutter') {
    return true;
  }
  ...
  if (/impeccable-flutter/i.test(content)) return true;
 ...
 * drive deletion even when SKILL.md never mentions impeccable-flutter.
...
export function removeDeprecatedSkills(projectRoot, lock) {
  ...
  for (const skillsDir of skillsDirs) {
    for (const name of targets) {
      ...
      // Regular directory -- verify it belongs to impeccable-flutter
 ...
 * Only removes entries whose source is "pbakaus/impeccable-flutter".
...
export function cleanSkillsLock(projectRoot) {
  ...
  for (const name of targets) {
    ...
    if (!entry) continue;
    // Only remove if it belongs to impeccable-flutter
    if (entry.source === 'pbakaus/impeccable-flutter') {
